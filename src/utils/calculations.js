...
export const calcPositionLiquidationPrice = (pos) => {
	if(pos.amount>0){
		if(pos.orderType==='0'){
		    if(pos.leverage>1){
		      	return Math.floor(((100-BANCRUPCY_DIFF)*(pos.price*DECIMALS/(1-1/pos.leverage)-pos.price*DECIMALS)/100+pos.price*DECIMALS)/DECIMALS);
		    } else return DECIMALS;
		    
		} else {  
		    return Math.floor((pos.price*DECIMALS-(pos.price*DECIMALS*(50/pos.leverage)/100)*(100-BANCRUPCY_DIFF)/100)/DECIMALS);
		}
	} else {
		return DECIMALS;
	}
